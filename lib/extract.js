...
function preserveWorksheetMeta(worksheet, newWorksheet, rows) {
...
function writeWorkbook(workbook) {
  try {
    ...
    return { outputFile: EXCEL_FILE, isFallbackCopy: false };
  } catch (error) {
...
async function processAllPdfs({ moveToBackup = true } = {}) {
  ...
  const { workbook, worksheet, rows } = readWorksheetRows(EXCEL_FILE);
  const results = [];
  const processedPdfs = [];
  ...
  const { outputFile, isFallbackCopy } = writeWorkbook(workbook);
